...
We can compute the $95\%$ confidence interval for the weight effect using the T-test, remembering that the degrees of freedom for the t student distribution are given by $df = n - p$ (in this case $n = 173$ and $p = 5$, including the intercept) and that we must then compute the exponential for this value since the effect is measured in log(odds).
```{r}
ConfInterval <- fit$coefficients[2] + c(-1,1)* summary(fit)$coefficients[2,2] * qt(0.975, df = (173-5))
exp(ConfInterval)
```
